"""
AWS Lambda Handler — Raster Line Extraction Microservice

Triggered by the Raster-Processing-Queue (SQS). Downloads a high-resolution
image from S3 (flattened PDF/PNG/TIFF), applies the full OpenCV pipeline
(grayscale → Gaussian blur → Canny edges → morphological close → Hough lines
→ affine correction), and outputs standardized JSON conforming to the
raw-coordinates.schema.json data contract.

Pipeline per SQS record:
  1. Parse SQS message → extract S3 URI + file metadata.
  2. Download image from S3 to Lambda /tmp storage.
  3. Run OpenCV pipeline → detect + affine-correct line segments.
  4. Deduplicate endpoints, build line/point references.
  5. Output strictly typed JSON matching raw-coordinates.schema.json.

Environment Variables:
  MAX_FILE_SIZE_MB      — Maximum image file size in MB (default: 200)
  LOG_LEVEL             — Logging verbosity (default: INFO)
  GAUSSIAN_KERNEL_SIZE  — Gaussian blur kernel (default: 5)
  CANNY_AUTO_THRESHOLD  — Use Otsu auto-threshold (default: true)
  HOUGH_THRESHOLD       — Hough vote threshold (default: 80)
  HOUGH_MIN_LINE_LENGTH — Minimum line length in pixels (default: 50)
  HOUGH_MAX_LINE_GAP    — Maximum gap between segments (default: 10)
  MIN_LINE_LENGTH       — Minimum output line length (default: 20)
  COORDINATE_PRECISION  — Decimal places for point dedup (default: 2)
...
def download_image_from_s3(bucket: str, key: str) -> str:
    """Download an image file from S3 to Lambda /tmp storage.
    ...
    Returns:
        Local file path of the downloaded image file.
    ...
    try:
        ...
        if content_length > max_bytes:
            raise S3DownloadError(
                message=(
                    f"Image file exceeds maximum allowed size: "
        ...
        tmp_fd, tmp_path = tempfile.mkstemp(suffix=suffix, prefix="raster_")
        try:
            ...
            with os.fdopen(tmp_fd, "wb") as f:
                while True:
                    ...
                    if bytes_written > max_bytes:
                        raise S3DownloadError(
                            message=(
                                f"Image file exceeded size limit during download: "
...
def parse_sqs_message(record: dict[str, Any]) -> dict[str, Any]:
    ...
    the file_ingestion Lambda, which includes:
      - storageUri: S3 URI of the image file
      - fileId: UUID of the uploaded file
      - fileName: Original filename
      - mimeType: Should be a raster MIME type
...
def process_sqs_record(record: dict[str, Any], request_id: str) -> dict[str, Any]:
    """Process a single SQS record through the full CV extraction pipeline.

    Pipeline stages:
      1. Parse SQS message → extract S3 URI + metadata.
      2. Download image from S3 to /tmp.
      3. Run OpenCV pipeline (blur → Canny → Hough → affine correction).
    ...
    Raises:
        ...
        EmptyImageError: If no lines are detected.
    ...
    file_name = message.get("fileName", "unknown.png")

    logger.info(
        "Processing raster line extraction request",
        extra=_log_extra(
    ...
    # ── Stage 2: Download image from S3 ──────────────────────────────────
    tmp_path = None
    try:
        tmp_path = download_image_from_s3(bucket, key)

        # ── Stage 3: Run OpenCV pipeline ─────────────────────────────────
        result = extract_lines(tmp_path)
        ...
        logger.info(
            "Raster line extraction completed successfully",
            extra=_log_extra(
                request_id=request_id,
                file_id=file_id,
                file_name=file_name,
                detected_lines=len(result.lines),
                unique_points=len(result.points),
                affine_corrected=result.pipeline_stats.get("lines_affine_corrected", 0),
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    """AWS Lambda entry point for Raster-Processing-Queue (SQS) processing.
    ...
    function_name = getattr(context, "function_name", "raster-line-extraction")
    ...
    for idx, record in enumerate(records):
        ...
        try:
            ...
            results.append({
                ...
                "affineCorrected": result.get("pipelineStats", {}).get(
                    "lines_affine_corrected", 0
                ),
                "data": result,
            })
            ...
            logger.error(
                "Image processing error during pipeline execution",
                extra=_log_extra(
                    record_id=record_id,
                    error=str(exc),
            ...
            error_detail = {
                "recordId": record_id,
                "status": "EMPTY_IMAGE_ERROR",
                "error": exc.to_dict(),
            }
            errors.append(error_detail)
            results.append(error_detail)
            logger.warning(
                "Image contained no detectable lines",
                extra=_log_extra(
                    record_id=record_id,
                    error=str(exc),
                    pipeline_stats=exc.pipeline_stats,
                ),
            )
